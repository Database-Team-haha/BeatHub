...
from music.models import LikeHistory, Song, User
        ...
        try:
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    liked_at_str = row['liked_at']

                    # Parse liked_at to a valid date object
                    liked_at = parse_date(str(liked_at_str))
                    if not liked_at:
                    ...
                    like, created = LikeHistory.objects.get_or_create(
                        id = index+1,
                        song=song, 
                        user=user, 
                        liked_at=liked_at
                    ...
                    if created:
                        ...
                        self.stdout.write(self.style.WARNING(f"Like already exists: {user.username} -> {song.title}"))
